...
import { requestOTP, verifyOTP, completeOnboarding } from '../src/api';
...
const AuthPage: React.FC<AuthPageProps> = ({ setUser }) => {
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4 font-heading">
      <div className="max-w-md w-full relative">
        {/* Background Glow */}
        <div className="absolute -inset-1 bg-gradient-to-r from-green-500 to-emerald-600 rounded-3xl blur opacity-20 animate-pulse"></div>
        
        <div className="relative bg-zinc-950 border border-white/10 p-10 rounded-3xl shadow-2xl">
          <div className="flex flex-col items-center mb-12">
            <div className="bg-green-500 p-4 rounded-2xl mb-6 shadow-[0_0_20px_rgba(34,197,94,0.3)]">
            ...
            <p className="text-zinc-500 text-[10px] mt-2 font-black uppercase tracking-[0.3em]">
              {step === 3 ? 'Finalizing Protocol' : 'Identity Protocol 2.5'}
            </p>
          </div>

          {error && (
            <div className="mb-6 p-3 bg-red-500/20 border border-red-500/50 text-red-500 text-xs font-bold text-center rounded-lg">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleAuth} className="space-y-6">
            {step === 1 ? (
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-black">Credential Source</label>
              ...
              <div className="space-y-6">
                <p className="text-center text-xs text-zinc-400 font-medium">We've dispatched a unique 6-digit signal to {phone}. Enter it below.</p>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] uppercase tracking-widest text-zinc-500 font-black">Full Name</label>
                  <div className="relative">
            ...
            <button 
              type="submit"
              disabled={loading}
              className="w-full bg-white text-black py-4 rounded-xl font-black uppercase tracking-[0.2em] hover:bg-green-500 transition-all flex items-center justify-center space-x-2 disabled:opacity-50"
            >
              <span>{loading ? 'Processing...' : (step === 1 ? 'Get OTP' : step === 2 ? 'Finalize Verification' : 'Initialize Account')}</span>
          ...
          <div className="mt-8 text-center">
            ...
            {step === 1 && (
              <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
                A unique ID will be created for new users
              </p>
            )}
          </div>
        </div>
