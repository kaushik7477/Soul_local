                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-xl transition-all border border-transparent ${
                      activeSection === item.id 
                        ? 'bg-zinc-800 border-zinc-700 text-white shadow-lg shadow-black/50' 
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      {item.badge !== undefined && item.badge > 0 && (
                        <span className="bg-green-500 text-black text-[10px] font-black px-2 py-0.5 rounded-full">{item.badge}</span>
                      )}
                    </div>
               ...
               <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-3xl">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm font-bold text-zinc-300">+91 {user.phone}</p>
                  </div>
                </div>
              </div>

              {/* Saved Locations Widget */}
              <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-3xl relative">
                ...
                {sortedAddresses.length > 0 ? (
                  <div className="bg-black/40 p-4 rounded-xl border border-zinc-800/50">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold uppercase tracking-wide text-zinc-300">{sortedAddresses[savedLocationIndex].receiverName}</p>
                ...
                <AnimatePresence mode="wait">
                  {activeOrders.length > 0 ? (
                    <motion.div
                      ...
                      className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl shadow-black/50"
                    >
                                 ...
                                 return (
                                   <div className="flex flex-col gap-1">
                                      ...
                                      {exchange.pickupTrackingId && (
                                        <p className="text-[10px] font-mono text-zinc-500 bg-zinc-800 px-2 py-0.5 rounded">
                      ...
                      <div className="flex flex-col gap-2 w-full md:w-auto">
                        <Link 
                          ...
                          className="bg-white text-black px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-green-500 transition-all text-center"
                        >
                          Details
                        </Link>
                    ...
                    <div className="bg-zinc-900/30 border border-zinc-800 p-6 rounded-3xl flex flex-col md:flex-row justify-between items-center gap-6 opacity-60 hover:opacity-100 transition-opacity">
                       <div className="flex items-center space-x-6 w-full">
                        ...
                        <div>
                          ...
                          <h4 className="font-bold uppercase tracking-tight text-zinc-400">
                      ...
                      <Link to="/products" className="px-6 py-2 bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-green-500 transition-all rounded-lg">
                        Order Again
                      </Link>
                    </div>
                  ) : (
                    <div className="bg-zinc-900/30 border border-dashed border-zinc-800 p-8 rounded-3xl flex flex-col items-center justify-center text-center">
                    ...
                    <div className="space-y-4">
                      {orders.map(order => (
                        <Link 
                          ...
                          className="bg-black/40 border border-zinc-800 rounded-2xl p-6 hover:border-zinc-600 transition-colors group block"
                        >
                            ...
                            <div className="flex gap-4">
                              ...
                              <div>
                                <div className="flex items-center gap-3 mb-1">
                                  <span className="text-xs font-bold text-white">#{(order.id || (order as any)._id || '').slice(-6)}</span>
              ...
              {activeSection === 'addresses' && (
                <div>
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button 
                      className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest bg-white text-black px-4 py-2 rounded-lg hover:bg-green-500 transition-colors"
                      onClick={() => {
                        setEditingAddress(null);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {user.addresses.map(address => {
                      ...
                      return (
                        <div key={id} className={`p-6 rounded-2xl border ${address.isDefault ? 'bg-zinc-900/50 border-green-500/50' : 'bg-black/40 border-zinc-800'} relative group`}>
                           {address.isDefault && (
                             <div className="absolute top-4 right-4 bg-green-500 text-black text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider">
                               Default
                             </div>
                           )}
                           <div className="flex items-start gap-4 mb-4">
                             <div className={`p-3 rounded-xl ${address.isDefault ? 'bg-green-500/10 text-green-500' : 'bg-zinc-800 text-zinc-400'}`}>
                           ...
                           <div className="flex items-center gap-2 mt-4 pt-4 border-t border-zinc-800/50">
                              {!address.isDefault && (
                                <button 
                                  ...
                                  className="flex-1 py-2 text-xs font-bold uppercase tracking-wider text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors"
                                >
                                  Set Default
                                </button>
                              )}
              ...
              {activeSection === 'settings' && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     {/* Profile Settings */}
                     <div className="space-y-6">
                        <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800 pb-2">Profile Information</h3>
                        <div className="space-y-4">
                           <div>
                             ...
                             <input 
                               ...
                               className="w-full bg-black border border-zinc-800 rounded-xl p-3 text-sm focus:border-green-500 outline-none transition-colors" 
                             />
                           </div>
                           <div>
                             ...
                             <div className="flex gap-2">
                               <input 
                                 ...
                                 className="w-full bg-black border border-zinc-800 rounded-xl p-3 text-sm focus:border-green-500 outline-none transition-colors" 
                               />
                               {/* <button className="bg-zinc-800 px-4 rounded-xl text-xs font-bold uppercase hover:bg-zinc-700">Verify</button> */}
                             </div>
                           </div>
                           <button 
                             onClick={handleProfileUpdate}
                             className="w-full bg-white text-black font-bold uppercase tracking-widest py-3 rounded-xl hover:bg-green-500 transition-colors"
                           >
                             Save Changes
                           </button>
                        </div>
                     ...
                     {/* Other Settings */}
                     <div className="space-y-6">
                        <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 border-b border-zinc-800 pb-2">Preferences & Security</h3>
                        <div className="space-y-3">
                           <button 
                             onClick={() => setNotificationsEnabled(!notificationsEnabled)}
                             className="w-full flex items-center justify-between p-4 bg-black/40 border border-zinc-800 rounded-xl hover:border-zinc-600 transition-colors group"
                           >
                              <div className="flex items-center gap-3">
                                 <Bell className="w-4 h-4 text-zinc-500 group-hover:text-green-500" />
                                 <span className="text-sm font-medium">Notifications</span>
                              </div>
                              <div className={`w-8 h-4 bg-zinc-800 rounded-full relative transition-colors ${notificationsEnabled ? 'bg-zinc-700' : ''}`}>
                               ...
                               {deleteAccountStep === 'initial' ? (
                                   ...
                                   <div className="space-y-3 bg-red-500/5 p-4 rounded-xl border border-red-500/20">
                                       ...
                                       <input 
                                         ...
                                         className="w-full bg-black border border-red-500/30 rounded-lg p-2 text-sm text-white focus:border-red-500 outline-none"
                                       />
                                       <div className="flex gap-2">
                                           <button 
                                             onClick={handleDeleteAccount}
                                             className="flex-1 bg-red-500 text-black font-bold uppercase text-xs py-2 rounded-lg hover:bg-red-400"
                                           >
                                             Confirm Delete
                                           </button>
                                           <button 
                                             ...
                                             className="px-4 bg-zinc-800 text-white font-bold uppercase text-xs py-2 rounded-lg hover:bg-zinc-700"
                                           >
                                             Cancel
                                           </button>
                                       </div>
        ...
        {showAddressModal && (
          ...
          >
            <motion.div 
              ...
              className="bg-zinc-900 border border-zinc-800 p-8 rounded-3xl max-w-lg w-full relative"
            >
              <button 
              ...
              <form onSubmit={handleAddressSubmit} className="space-y-4">
                <div>
                   <label className="block text-xs text-zinc-400 mb-1 uppercase tracking-wider">Receiver Name</label>
                   <input required name="receiverName" defaultValue={editingAddress?.receiverName} className="w-full bg-black border border-zinc-800 rounded-xl p-3 text-sm focus:border-green-500 outline-none transition-colors" />
                </div>
                <div>
                   <label className="block text-xs text-zinc-400 mb-1 uppercase tracking-wider">Flat / House No / Building</label>
                   <input required name="apartment" defaultValue={editingAddress?.apartment} className="w-full bg-black border border-zinc-800 rounded-xl p-3 text-sm focus:border-green-500 outline-none transition-colors" />
                </div>
                <div>
                   <label className="block text-xs text-zinc-400 mb-1 uppercase tracking-wider">Street / Colony / Area</label>
                   <input required name="roadName" defaultValue={editingAddress?.roadName} className="w-full bg-black border border-zinc-800 rounded-xl p-3 text-sm focus:border-green-500 outline-none transition-colors" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                     <label className="block text-xs text-zinc-400 mb-1 uppercase tracking-wider">City</label>
                     <input required name="city" defaultValue={editingAddress?.city} className="w-full bg-black border border-zinc-800 rounded-xl p-3 text-sm focus:border-green-500 outline-none transition-colors" />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                     <label className="block text-xs text-zinc-400 mb-1 uppercase tracking-wider">Pincode</label>
                     <input required name="pincode" defaultValue={editingAddress?.pincode} className="w-full bg-black border border-zinc-800 rounded-xl p-3 text-sm focus:border-green-500 outline-none transition-colors" />
                  </div>
                  {/* Phone number comes from OTP-verified login; remove phone field from address form */}
                </div>

                <div className="flex items-center gap-3 bg-zinc-800/30 p-3 rounded-xl border border-zinc-800">
                    <input type="checkbox" name="isDefault" id="isDefault" defaultChecked={editingAddress?.isDefault} className="w-4 h-4 rounded bg-zinc-800 border-zinc-700 text-green-500 focus:ring-green-500 accent-green-500" />
                    <label htmlFor="isDefault" className="text-xs font-bold uppercase tracking-wider text-zinc-300 cursor-pointer">Set as Default Address</label>
                </div>

                <div className="pt-4">
                  <button type="submit" className="w-full bg-white text-black font-bold uppercase tracking-widest py-3 rounded-xl hover:bg-green-500 transition-colors">
          ...
          >
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-zinc-400 text-sm">Select how you want to disconnect from the Soul Stich network.</p>
              </div>

              <div className="space-y-3">
                <button 
                  onClick={() => handleLogout(false)}
                  className="w-full py-4 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl font-bold uppercase tracking-widest text-xs transition-colors"
                >
                  Logout Current Device
                </button>
                <button 
                  onClick={() => handleLogout(true)}
                  className="w-full py-4 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-black border border-red-500/20 hover:border-transparent rounded-xl font-bold uppercase tracking-widest text-xs transition-colors"
                >
                  Logout All Devices
                </button>
              </div>
