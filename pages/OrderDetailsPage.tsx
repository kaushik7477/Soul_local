...
const OrderDetailsPage: React.FC<OrderDetailsPageProps> = ({ user, products }) => {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'exchanged': return 'text-green-500 bg-green-500/10';
  ...
  if (!order) return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4">
      ...
      <p className="text-zinc-500 mb-8">The order you're looking for doesn't exist in our archives.</p>
      <Link to="/profile" className="bg-white text-black px-8 py-3 rounded-full text-xs font-black uppercase tracking-widest hover:bg-green-500 transition-colors">Return to Base</Link>
    </div>
  );

  return (
    <div className="bg-black min-h-screen text-white pt-24 pb-20 font-sans">
      <div className="container mx-auto px-4 max-w-4xl">
        {/* Back Button */}
        <button onClick={() => navigate('/profile')} className="flex items-center gap-2 text-zinc-500 hover:text-white transition-colors mb-10 group">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-white/5 pb-12">
            <div>
              <div className="flex items-center gap-3 mb-3">
                {/* <span className="text-[10px] text-zinc-500 uppercase font-black tracking-widest">Transmission ID</span> */}
                {/* <span className="px-2 py-0.5 bg-zinc-900 border border-white/10 rounded text-[10px] font-mono text-zinc-300">{(order as any).orderCode || ('#' + (order.id || (order as any)._id).toUpperCase())}</span> */}
              </div>
              <h1 className="text-4xl font-black uppercase tracking-tighter">Order Details</h1>
              <p className="text-2xl font-black text-green-500 mt-2">{(order as any).orderCode || ('#' + (order.id || (order as any)._id))}</p>
              <p className="text-zinc-500 text-xs mt-2 uppercase tracking-widest font-bold">Placed on {new Date(order.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
            </div>
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              <button onClick={generateInvoice} className="flex items-center gap-2 text-zinc-400 hover:text-green-500 transition-colors text-[10px] font-black uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            ...
            <div className="md:col-span-2 space-y-10">
              <div className="space-y-6">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-500 border-l-2 border-green-500 pl-4">Manifest</h3>
                <div className="space-y-4">
                  {order.products.map((item, idx) => {
                    const prod = getProduct(item.productId);
                    return (
                      <div key={idx} className="bg-zinc-900/50 border border-white/5 rounded-3xl p-6 flex gap-6 hover:bg-zinc-900 transition-colors group">
                        <div className="w-24 h-32 bg-black rounded-2xl overflow-hidden flex-shrink-0 border border-white/10">
                        ...
                        <div className="flex-grow">
                          ...
                          <div className="flex flex-wrap gap-4 mt-3">
                            <div className="bg-zinc-800/50 px-3 py-1 rounded-lg">
                              <p className="text-[9px] text-zinc-500 uppercase font-black">Size</p>
                              <p className="text-sm font-bold">{item.size}</p>
                            </div>
                            <div className="bg-zinc-800/50 px-3 py-1 rounded-lg">
                              <p className="text-[9px] text-zinc-500 uppercase font-black">Quantity</p>
                              <p className="text-sm font-bold">{item.quantity}</p>
                            </div>
                            <div className="bg-zinc-800/50 px-3 py-1 rounded-lg">
                              <p className="text-[9px] text-zinc-500 uppercase font-black">Price</p>
                  ...
                  {order.exchangeRequest ? (
                    <div className="space-y-6">
                      <div className="flex items-center justify-between">
                        <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
                          ...
                          order.exchangeRequest.status === 'approved' ? 'bg-green-500/10 text-green-500' :
                      ...
                      <div className="bg-black/40 border border-white/5 rounded-2xl p-6 space-y-4">
                            ...
                            return (
                              <div key={i} className="relative z-10 flex flex-col items-center gap-2">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all duration-500 ${
                                  isActive ? 'bg-green-500 border-green-500 text-black shadow-[0_0_15px_rgba(34,197,94,0.3)]' : 'bg-zinc-900 border-white/10 text-zinc-500'
                                }`}>
                                  <step.icon className="w-4 h-4" />
                                </div>
                                <span className={`text-[8px] font-black uppercase tracking-widest ${isActive ? 'text-white' : 'text-zinc-600'}`}>{step.label}</span>
                              </div>
                            );
                          })}
                        </div>
                        ...
                        <div className="pt-4 border-t border-white/5 flex gap-4">
                          <div className="flex-1">
                            <p className="text-[9px] text-zinc-500 uppercase font-black mb-1">Reason</p>
                            <p className="text-xs text-zinc-300 italic">"{order.exchangeRequest.reason}"</p>
                          </div>
                          {order.exchangeRequest.adminNotes && (
                            <div className="flex-1 border-l border-white/5 pl-4">
                              <p className="text-[9px] text-zinc-500 uppercase font-black mb-1">Admin Update</p>
                              <p className="text-xs text-zinc-300">{order.exchangeRequest.adminNotes}</p>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {order.exchangeRequest.pickupTrackingId && (
                        <div className="p-4 bg-blue-500/5 border border-blue-500/10 rounded-2xl flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-[9px] text-zinc-500 uppercase font-black">Pickup Tracking</p>
                              <p className="text-xs font-mono text-white tracking-widest">{order.exchangeRequest.pickupTrackingId}</p>
                            </div>
                          </div>
                          <button className="text-[9px] font-black uppercase tracking-widest text-blue-500 hover:text-white transition-colors">Track Status</button>
                        </div>
                      )}
                    </div>
                  ) : isExchangeEligible ? (
                    <div className="space-y-6">
                      ...
                      <button 
                        ...
                        className="bg-green-500 text-black px-8 py-3 rounded-full text-xs font-black uppercase tracking-widest hover:bg-white transition-all shadow-lg shadow-green-500/20 flex items-center gap-2"
                      >
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="mt-4 p-4 bg-black/50 border border-white/5 rounded-2xl flex items-start gap-3">
                        <Info className="w-4 h-4 text-zinc-600 mt-0.5" />
                        <div>
                           <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Exchange Policy</p>
            ...
            <div className="space-y-10">
              ...
              <div className="bg-zinc-900/50 border border-white/5 rounded-3xl p-8 space-y-6">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-500 border-l-2 border-green-500 pl-4">Financials</h3>
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-white/5 flex justify-between items-end">
                    <span className="text-zinc-300 uppercase font-black tracking-widest text-xs">Total</span>
                    <span className="text-2xl font-black text-green-500">₹{order.totalAmount}</span>
                  </div>
                </div>
                <div className="pt-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-500">
                ...
                {order.status === 'cancelled' && (
                  <div className="mt-4 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-sm">
                    ...
                    <p className="text-zinc-300">Refund initiate into source account.</p>
                    <p className="text-zinc-400 mt-1">Amount: ₹{order.totalAmount}</p>
                    <p className="text-zinc-400">Refund ID: {(order as any).refundDetails?.refundId || 'N/A'}</p>
                  </div>
                )}
              </div>
              ...
              {/* Delivery Info */}
              <div className="bg-zinc-900/50 border border-white/5 rounded-3xl p-8 space-y-6">
                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-500 border-l-2 border-green-500 pl-4">Drop Location</h3>
                <div className="space-y-4">
                   ...
                   <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                      ...
                      <p className="text-xs text-zinc-400 font-bold">+91 {user.phone}</p>
                   </div>
                </div>
              </div>
        ...
        <div className="container mx-auto px-4 max-w-4xl mt-8">
          <div className="p-8 border border-white/5 rounded-3xl text-center space-y-4 mx-auto max-w-md">
            <Info className="w-8 h-8 text-zinc-800 mx-auto" />
            <p className="text-[10px] text-zinc-500 uppercase font-black tracking-widest">Need Assistance?</p>
            <p className="text-xs text-zinc-400">Encountered an issue with your transmission? Contact our support relay.</p>
            <a href="mailto:soulstich.store@gmail.com" className="inline-block text-[10px] font-black uppercase text-green-500 hover:text-white transition-colors">Contact Support</a>
          </div>
        </div>
      </div>
        ...
        {showExchangeModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-zinc-950 border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl"
            >
              {/* Modal Header */}
              <div className="p-8 border-b border-white/5 flex justify-between items-center bg-zinc-900/30">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mt-1">Step {exchangeStep === 'reason' ? '1' : exchangeStep === 'product' ? '2' : exchangeStep === 'address' ? '3' : '4'} of 4</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="space-y-4">
                      ...
                      <textarea 
                        ...
                        className="w-full bg-zinc-900 border border-white/10 rounded-2xl p-4 h-32 text-sm focus:border-green-500/50 transition-colors"
                      />
                      <div className="grid grid-cols-3 gap-2 pt-2">
                        {['Size too small', 'Size too large', 'Quality issue', 'Wrong item received', 'Color mismatch', 'Defect'].map(opt => (
                          <button
                            ...
                            className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border ${selectedReasons.includes(opt) ? 'bg-green-500 text-black border-green-500' : 'bg-zinc-900 text-zinc-300 border-white/10'}`}
                          >
                            {opt}
                          </button>
                        ))}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        {photoUrls.map((url, i) => (
                          <div key={i} className="aspect-square bg-zinc-900 rounded-xl overflow-hidden border border-white/10 relative group">
                        ...
                        {photoUrls.length < 4 && (
                          <label className="aspect-square bg-zinc-900/50 border-2 border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-zinc-900 hover:border-green-500/50 transition-all group">
                            <input type="file" multiple accept="image/*" className="hidden" onChange={handlePhotoUpload} />
                            {uploadingPhotos ? (
                              <RefreshCw className="w-6 h-6 text-green-500 animate-spin" />
                            ) : (
                              <>
                                <Upload className="w-6 h-6 text-zinc-600 group-hover:text-green-500 transition-colors" />
                    ...
                    <button 
                      ...
                      className="w-full bg-white text-black py-4 rounded-2xl font-black uppercase tracking-[0.2em] text-xs hover:bg-green-500 transition-all disabled:opacity-50 disabled:hover:bg-white"
                    >
                      Continue Protocol
                    </button>
                  </div>
                ...
                {exchangeStep === 'address' && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="space-y-4">
                      ...
                      <div className="bg-zinc-900/50 border border_white/10 rounded-2xl p-6">
                        <p className="text-[10px] text-zinc-500 uppercase font-black mb-2">Pickup Address (Original Delivery)</p>
                        ...
                        <p className="text-[10px] text-zinc-500 mt-1">Phone: {selectedAddress?.phone || user.phone}</p>
                    ...
                    <div className="flex gap-4 pt-2">
                      <button onClick={() => setExchangeStep('product')} className="flex-1 bg-zinc-900 text_white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-zinc-800 transition-all">Back</button>
                      <button 
                        onClick={() => setExchangeStep('confirm')}
                        className="flex-[2] bg-white text-black py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-green-500 transition-all"
                      >
                        Continue
                      </button>
                    </div>
                ...
                {/* {exchangeStep === 'address' && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="space-y-4">
                      ...
                      <div className="bg-zinc-900/50 border border-white/10 rounded-2xl p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Apartment / House</p>
                          <input 
                        ...
                        <div>
                          <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Road / Street</p>
                          <input 
                        ...
                        <div>
                          <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">City</p>
                          <input 
                        ...
                        <div>
                          <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">State</p>
                          <input 
                        ...
                        <div>
                          <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Pincode</p>
                          <input 
                        ...
                        <div>
                          <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Phone</p>
                          <input 
                    ...
                    <div className="flex gap-4 pt-2">
                      <button onClick={() => setExchangeStep('product')} className="flex-1 bg-zinc-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-zinc-800 transition-all">Back</button>
                      <button 
                        onClick={() => setExchangeStep('confirm')}
                        className="flex-[2] bg-white text-black py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-green-500 transition-all"
                      >
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        {products.filter(p => p.id === order.products[0].productId || p.linkedProducts?.includes(getProduct(order.products[0].productId)?.sku || '')).map(prod => (
                          <button 
                            ...
                            className={`p-4 rounded-2xl border transition-all text-left flex gap-4 ${
                                selectedExchangeProduct?.id === prod.id ? 'bg-green-500/10 border-green-500' : 'bg-zinc-900/50 border-white/5 hover:border-white/20'
                            }`}
                          >
                            ...
                            <div>
                                ...
                                <p className="text-xs font-bold text-green-500 mt-1">₹{prod.offerPrice}</p>
                              <p className="text-[10px] text-zinc-500 font-mono mt-1">SKU: {prod.sku}</p>
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    {selectedExchangeProduct && (
                      <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                        <label className="text-xs font-black uppercase tracking-widest text-zinc-500">Select Desired Size</label>
                          ...
                          {Object.entries(selectedExchangeProduct.sizes).map(([size, stock]) => (
                            <button 
                              ...
                              className={`px-6 py-3 rounded-xl text-xs font-black uppercase transition-all ${
                                stock === 0 ? 'bg-zinc-900 text-zinc-700 cursor-not-allowed opacity-30' :
                                selectedSize === size ? 'bg-green-500 text-black' : 'bg-zinc-800 text-white hover:bg-zinc-700'
                              }`}
                            >
                              {size}
                            </button>
                    ...
                    <div className="flex gap-4 pt-4">
                        <button onClick={() => setExchangeStep('reason')} className="flex-1 bg-zinc-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-zinc-800 transition-all">Back</button>
                        <button 
                            disabled={!selectedExchangeProduct || !selectedSize}
                            onClick={() => setExchangeStep('address')}
                            className="flex-[2] bg-white text-black py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-green-500 transition-all disabled:opacity-50"
                        >
                            Review Address
                        </button>
                    </div>
                ...
                {exchangeStep === 'confirm' && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-zinc-900/50 border border-white/10 rounded-3xl p-8 space-y-6">
                        ...
                        <div className="flex gap-8 items-center">
                            <div className="flex-1 space-y-2">
                                <p className="text-[10px] text-zinc-500 uppercase font-black">Current Artifact</p>
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-14 bg-black rounded-lg overflow-hidden border border-white/5">
                            ...
                            <div className="flex-1 space-y-2">
                                <p className="text-[10px] text-zinc-500 uppercase font-black">Exchange For</p>
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-14 bg-black rounded-lg overflow-hidden border border-green-500/20">
                                        <img src={selectedExchangeProduct?.images[0]} alt="" className="w-full h-full object-cover" />
                                    </div>
                                    <p className="text-xs font-bold uppercase text-green-500">{selectedSize}</p>
                                </div>
                                <p className="text-[10px] text-zinc-500 font-mono">SKU: {selectedExchangeProduct?.sku}</p>
                            </div>
                        </div>
                        ...
                        <div className="pt-6 border-t border-white/10 space-y-3">
                            <div className="flex justify-between text-xs">
                                <span className="text-zinc-500 uppercase font-bold">Price Difference</span>
                                {(() => {
                        ...
                        <div className="pt-6 border-t border-white/10">
                          <p className="text-[10px] text-zinc-500 uppercase font-black mb-2">Pickup Address</p>
                    ...
                    <div className="flex gap-4">
                        <button onClick={() => setExchangeStep('address')} className="flex-1 bg-zinc-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-zinc-800 transition-all">Back</button>
                        <button 
                            onClick={submitExchangeRequest}
                            className="flex-[2] bg-green-500 text-black py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-white transition-all shadow-xl shadow-green-500/20"
                        >
                            Finalize Request
                        </button>
                    </div>
